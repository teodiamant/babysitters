    ...
    pastJobs.map((job) => (
      <Card key={job.id} sx={{ mb: 2 }}>
        <CardContent>
          ...
          <Box sx={{ mt: 2 }}>
            <Typography variant="body1">Rate this job:</Typography>
            <Rating
              name={`rating-${job.id}`}
              max={5}
              onChange={(event, value) => {
                job.tempRating = value; // Αποθηκεύουμε προσωρινά τη βαθμολογία
              }}
            />
            <TextField
              ...
              sx={{ mt: 2 }}
              onChange={(e) => {
                job.tempComment = e.target.value; // Αποθηκεύουμε προσωρινά το σχόλιο
              }}
            />
            <Button
              variant="contained"
